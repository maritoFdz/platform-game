...
public enum WallJump { Front, Climb }

public class WallJumpState : IPlayerState
{
    private int direction;
    public WallJump jumpType;
    ...
    public void EnterState(Player player)
    {
        ...
        Vector2 jump;
        if (jumpType == WallJump.Front)
            jump = player.playerParameters.frontDirectionJump;
        else
            jump = player.playerParameters.climbJump;
        player.velocity = new Vector2(jump.x * -direction, jump.y);
    }
